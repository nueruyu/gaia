...
from gaia.application.dtos import SessionDto, ToolDefinitionDto, ToolOutputDto, ToolCallDto, PlanDto

class ToolMapper:
    @staticmethod
    def to_domain(dto: ToolDefinitionDto) -> ToolDefinition:
        return ToolDefinition(name=dto.name, description=dto.description, parameters=dto.parameters)
    ...
    def to_dto(session: PlanningSession) -> SessionDto:
        ...
        if session.status == SessionStatus.WAITING_FOR_TOOL:
            ...
            if isinstance(last_msg, AIMessage) and last_msg.tool_calls:
                tool_calls_dto = [
                    ToolCallDto(id=tc.id, function_name=tc.function_name, arguments=json.dumps(tc.arguments))
        ...
        if session.generated_plan:
            ...
            plan_dto = PlanDto(
                ...
                objectives=[{"type": o.type, "parameters": o.parameters, "priority": o.priority} for o in p.objectives],
                strategy={
